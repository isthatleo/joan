...
export default function PublicTenantLoginPage() {
  ...
  useEffect(() => {
    const verifyTenant = async () => {
      try {
        ...
        setTenantName(tenant.name || slug);
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-subtle p-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">The hospital "{slug}" does not exist or is not currently available.</p>
          <Link href="/login" className="mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-primary-foreground font-semibold hover:opacity-90 transition-opacity">
            ← Back to Login
          </Link>
        </div>
      </div>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
          <div className="mb-6 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Sign in to {tenantName}</p>
          </div>
