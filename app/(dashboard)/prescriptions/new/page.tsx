...
export default function NewPrescriptionPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (payload) => {
      ...
      router.push(`/prescriptions/${payload.prescription.id}`);
    },
    onError: (error) => {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3">
          <Link href="/prescriptions" className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted/40">
            Cancel
          </Link>
          <button
            type="submit"
