...
export default function DoctorFeedbackPage() {
  ...
  const { data: feedbackData, isLoading } = useQuery({
    queryKey: ["doctor-feedback", statusFilter, typeFilter],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (statusFilter !== "all") params.set("status", statusFilter);
      if (typeFilter !== "all") params.set("type", typeFilter);

      const response = await fetch(`/api/doctor/feedback?${params.toString()}`, { cache: "no-store" });
  ...
  const submitFeedbackMutation = useMutation({
    ...
    onSuccess: () => {
      setIsCreating(false);
      setNewFeedback({ type: "service", title: "", description: "", priority: "medium" });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="System bugs and product requests route to the super admin. Tenant service and operational issues route to hospital administration."
        actions={
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <SectionCard>
          <div className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">Routing</p>
            <h2 className="text-base font-semibold text-foreground">Hospital administration</h2>
            <p className="text-sm text-muted-foreground">
              Service delivery issues, local workflow gaps, and operational escalations stay inside the active tenant.
            </p>
          </div>
        </SectionCard>
        <SectionCard>
          <div className="space-y-2">
            ...
            <h2 className="text-base font-semibold text-foreground">Product and engineering</h2>
            <p className="text-sm text-muted-foreground">
              Bugs, feature additions, and product improvements go straight to the super admin for implementation review.
            </p>
          </div>
        </SectionCard>
        <SectionCard>
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Capture the page, patient workflow, expected behavior, actual behavior, and urgency so support can act without back-and-forth.
            </p>
          </div>
        </SectionCard>
      </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Service Issue stays with tenant admins. Bug, Feature Request, and Improvement go to the super admin.
              </p>
            </div>
