...
import { Wallet, Search, Plus, DollarSign, Receipt, CreditCard, AlertCircle, CheckCircle, Loader2, Clock } from "lucide-react";
...
export default function BillingPage() {
  ...
  const filteredInvoices = invoices.filter(i => {
    const matchesSearch = i.patientId.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const totalRevenue = invoices.filter(i => i.status === "paid").reduce((sum, i) => sum + Number(i.totalAmount), 0);
  const pendingAmount = invoices.filter(i => i.status === "pending").reduce((sum, i) => sum + Number(i.totalAmount), 0);
  const overdueAmount = invoices.filter(i => i.status === "overdue").reduce((sum, i) => sum + Number(i.totalAmount), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <p className="text-xs font-mono text-muted-foreground uppercase tracking-wider">Financial Management</p>
          <h1 className="text-3xl font-bold text-foreground mt-1">Billing & Invoices</h1>
          <p className="text-sm text-muted-foreground mt-1">Manage patient billing and payment tracking.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-3">
            <div className="size-10 rounded-full bg-yellow-50 dark:bg-yellow-500/10 flex items-center justify-center text-yellow-500">
              <Clock className="size-5" />
            </div>
            <div>
              <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide">Pending</p>
              <p className="text-2xl font-semibold text-foreground">${pendingAmount.toFixed(2)}</p>
            </div>
          </div>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-foreground">${overdueAmount.toFixed(2)}</p>
            </div>
          </div>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              placeholder="Search invoices by patient, description, or ID..."
                  ...
                  <tr key={i.id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-5 py-3 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-foreground font-mono">#{i.id}</p>
                    ...
                    <td className="px-5 py-3 whitespace-nowrap">
                      <div>
                        <p className="font-semibold text-foreground">{i.patientId}</p>
                      </div>
                    </td>
                    <td className="px-5 py-3 whitespace-nowrap">
                      <p className="text-lg font-semibold text-foreground">${Number(i.totalAmount).toFixed(2)}</p>
                    </td>
                    <td className="px-5 py-3 whitespace-nowrap">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${getStatusColor(i.status)}`}>
                        {getStatusIcon(i.status)}
                        {i.status.charAt(0).toUpperCase() + i.status.slice(1)}
                      </span>
                    </td>
                    <td className="px-5 py-3 whitespace-nowrap">
                      <p className="text-sm text-foreground">{new Date(i.updatedAt).toLocaleDateString()}</p>
                    </td>
                    <td className="px-5 py-3 whitespace-nowrap">
