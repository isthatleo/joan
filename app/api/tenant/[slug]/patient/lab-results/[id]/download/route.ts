...
export async function GET(request: NextRequest, { params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  if (!result.patientPortalEligible) {
    return NextResponse.json({ error: "Payment is required before downloading this result" }, { status: 403 });
  }
