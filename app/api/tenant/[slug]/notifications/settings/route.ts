...
export async function GET(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    return NextResponse.json({
      ...
      overview,
      channelAvailability: {
        email: activeProviders.some((provider) => ["resend", "sendgrid", "mailgun", "aws-ses", "smtp"].includes(provider)),
        sms: activeProviders.some((provider) => ["twilio", "nexmo", "vonage", "messagebird", "aws-sns"].includes(provider)),
    ...
    console.error("[tenant notifications settings:get]", error);
    return NextResponse.json({ error: error?.message || "Failed to load notification settings" }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    console.error("[tenant notifications settings:put]", error);
    return NextResponse.json({ error: error?.message || "Failed to save notification settings" }, { status: 500 });
  }
}
