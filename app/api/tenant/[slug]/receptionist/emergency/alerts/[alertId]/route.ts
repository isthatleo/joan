...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ slug: string; alertId: string }> }) {
  try {
    const { slug, alertId } = resolvedParams;
    const { status, assignedTeam } = await request.json();

    // Mock alert update - replace with actual database operations
    console.log(`Updating emergency alert ${alertId}: status=${status}, team=${assignedTeam}`);
