...
async function upsertPreferences(tenantId: string, value: Record<string, any>) {
  ...
  await db.insert(tenantSettings).values({ tenantId, key: "preferences", value });
}
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  await upsertPreferences(access.tenant.id, preferences);
