  ...
  useEffect(() => {
    if (broadcastsData?.broadcasts && broadcastsData.broadcasts.length > 0) {
      broadcastsData.broadcasts.slice(0, 3).forEach((broadcastRead: any) => {
        ...
        // Show Sonner toast with broadcast info
        toast.info(
          <div className="flex flex-col gap-1">
            <p className="font-semibold text-sm">{broadcastInfo.title}</p>
            <p className="text-xs text-muted-foreground">{broadcastInfo.message}</p>
            <p className="text-xs text-muted-foreground">
              From: {broadcastInfo.sender?.fullName || broadcastInfo.sender?.email}
            </p>
          </div>,
          {
            position: "top-right",
