...
export function normalizeTenantSystemSettings(value?: Partial<TenantSystemSettings> | Record<string, any> | null): TenantSystemSettings {
  ...
  return {
    ...
    cpuThreshold: typeof source.cpuThreshold === "number" && Number.isFinite(source.cpuThreshold) ? source.cpuThreshold : DEFAULT_TENANT_SYSTEM_SETTINGS.cpuThreshold,
    memoryThreshold: typeof source.memoryThreshold === "number" && Number.isFinite(source.memoryThreshold) ? source.memoryThreshold : DEFAULT_TENANT_SYSTEM_SETTINGS.memoryThreshold,
    diskThreshold: typeof source.diskThreshold === "number" && Number.isFinite(source.diskThreshold) ? source.diskThreshold : DEFAULT_TENANT_SYSTEM_SETTINGS.diskThreshold,
...
export async function upsertTenantSystemSettings(tenantId: string, value: TenantSystemSettings) {
  ...
  await db.insert(tenantSettings).values({ tenantId, key: "system", value });
}
